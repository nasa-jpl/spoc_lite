...
// Interface functions
//
model* svm_linear_train(const problem *prob, const parameter *param)
{
	int i,j;
	int l = prob->l;
	int n = prob->n;
	...
	if(svm_linear_check_regression_model(model_))
	{
...
void svm_linear_cross_validation(const problem *prob, const parameter *param, int nr_fold, double *target)
{
	int i;
	int *fold_start;
	int l = prob->l;
	...
	for(i=0;i<nr_fold;i++)
	{
		...
		struct model *submodel = svm_linear_train(&subprob,param);
		for(j=begin;j<end;j++)
			target[perm[j]] = svm_linear_predict(submodel,prob->x[perm[j]]);
		svm_linear_free_and_destroy_model(&submodel);
		free(subprob.x);
		free(subprob.y);
	}
	free(fold_start);
...
void svm_linear_find_parameter_C(const problem *prob, const parameter *param, int nr_fold, double start_C, double max_C, double *best_C, double *best_rate)
{
	// variables for CV
	int i;
	int *fold_start;
	...
	*best_rate = 0;
	...
	while(param1.C <= max_C)
	{
		...
		svm_linear_set_print_string_function(&print_null);
		...
		for(i=0; i<nr_fold; i++)
		{
			...
			struct model *submodel = svm_linear_train(&subprob[i],&param1);
		...
		svm_linear_set_print_string_function(default_print_string);
...
double svm_linear_predict(const model *model_, const feature_node *x)
{
	double *dec_values = Malloc(double, model_->nr_class);
	double label=svm_linear_predict_values(model_, x, dec_values);
	free(dec_values);
	return label;
}
...
{
	if(svm_linear_check_probability_model(model_))
	{
		int i;
		int nr_class=model_->nr_class;
		int nr_w;
		...
		double label=svm_linear_predict_values(model_, x, prob_estimates);
		for(i=0;i<nr_w;i++)
...
int svm_linear_get_nr_feature(const model *model_)
{
	return model_->nr_feature;
}

int svm_linear_get_nr_class(const model *model_)
{
	return model_->nr_class;
}

void svm_linear_get_labels(const model *model_, int* label)
{
...
void svm_linear_free_and_destroy_model(struct model **model_ptr_ptr)
{
	...
	if(model_ptr != NULL)
	{
		svm_linear_free_model_content(model_ptr);
		free(model_ptr);
	}
}

void svm_linear_destroy_param(parameter* param)
{
...
const char *svm_linear_check_parameter(const problem *prob, const parameter *param)
{
	if(param->eps <= 0)
		return "eps <= 0";
...
int svm_linear_check_probability_model(const struct model *model_)
{
...
int svm_linear_check_regression_model(const struct model *model_)
{
